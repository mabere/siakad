<x-main-layout>
    ...
    <div class="nk-block-head nk-block-head-sm">
        <h3 class="nk-block-title page-title">Tambah Mata Kuliah: {{
    ...
    <div class="nk-block">
        <div class="card card-bordered">
            <div class="card-inner">
                ...
                <form action="{{ route('curriculums.courses.store', $curriculum) }}" method="POST"
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <select name="semester_number" id="semester_number" class="form-select" required>
                                    @for($i = 1; $i <= 8; $i++) <option value="{{ $i }}" {{ old('semester_number')==$i
                                        ? 'selected' : '' }}>{{ $i }}</option>
                                        @endfor
                                </select>
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <select name="kategori" id="kategori" class="form-select" required>
                                    ...
                                    <option value="Tugas AKhir" {{ old('kategori')==='Tugas Akhir' ? 'selected' : '' }}>
                                        SKRIPSI
                                    </option>
                                </select>
                                @error('kategori')
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <select name="prerequisites[]" id="prerequisites" class="form-select" multiple>
                                    ...
                                    <option value="{{ $course->id }}" {{ in_array($course->id, old('prerequisites', []))
                                        ? 'selected' : '' }}>
    ...
    <script>
        document.querySelector('form').addEventListener('submit', function(e) {
            e.preventDefault();
            if (confirm('Apakah Anda yakin ingin menambahkan mata kuliah?')) {
                this.submit();
            }
        });
    </script>
